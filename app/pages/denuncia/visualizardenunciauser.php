...
if ($id_denuncia) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Consulta para buscar a resposta da denúncia
        $sql_resposta = "SELECT descricao FROM retorno WHERE id_denuncia_fk = ?";
        $stmt_resposta = $mysqli->prepare($sql_resposta);
        $stmt_resposta->bind_param("i", $id_denuncia);
...
<main>
  <div class="container">
    ...
    <div class="section">
      <h2>Resposta da Denúncia</h2>
      <?php if ($resposta): ?>
        <p><strong>Resposta:</strong> <?php echo $resposta['descricao']; ?></p>
      <?php else: ?>
